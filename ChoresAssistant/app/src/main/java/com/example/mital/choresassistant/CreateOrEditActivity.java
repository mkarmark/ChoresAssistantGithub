...
public class CreateOrEditActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        personID = getIntent().getIntExtra(PostChore.KEY_EXTRA_CONTACT_ID, 0);
        ...
        if(personID > 0) {
            ...
            rs.moveToFirst();
            String personName = rs.getString(rs.getColumnIndex(FeedReaderDBHelper.PERSON_COLUMN_NAME));
            String personGender = rs.getString(rs.getColumnIndex(FeedReaderDBHelper.PERSON_COLUMN_GENDER));
            int personAge = rs.getInt(rs.getColumnIndex(FeedReaderDBHelper.PERSON_COLUMN_AGE));
            if (!rs.isClosed()) {
                rs.close();
            }
